...
const TvShows = ({ query = "" }) => { // ✅ default empty string
  const [tv, setTv] = useState([]);
  const [type, setType] = useState("popular");
  const [page, setPage] = useState(1);
  const [searchResult, setSearchResult] = useState([]);

  // Fetch default type TV shows
  useEffect(() => {
    if (query.trim() !== "") return; // skip default fetch during search
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetch(`${BASE_URL}/search/tv?api_key=${API_KEY}&query=${query}&page=1`)
        .then((res) => res.json())
  ...
  // Show search results if searching, else default tv list
  const displayTv = searchResult.length > 0 ? searchResult : tv;
  ...
  return (
    <div className="p-3 mt-8 bg-[#0f0f0f] text-[#d3d3d3]">
      ...
      <div className="flex flex-wrap justify-center gap-3 mb-5">
        {["popular", "airing_today", "on_the_air", "top_rated"].map((btn) => (
          <button
            key={btn}
        ...
        {displayTv.map((series) => (
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none bg-black/80 text-white flex flex-col justify-center items-center p-3 opacity-0 group-hover:opacity-100 transition duration-300 rounded-1xl">
              ...
              <p className="text-sm">
                Release: {series.first_air_date?.slice(0, 4)}
              </p>
              <p className="text-sm mt-0.5">
                Overview: {series.overview?.slice(0, 100)}...
              </p>
            </div>
            ...
            <button
              onClick={() =>
                window.open(
                  `https://vidsrc.win/watch/${series.id}?s=1&e=1`,
                  "_blank"
                )
              }
